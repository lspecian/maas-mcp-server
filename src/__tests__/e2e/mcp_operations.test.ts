...
const { MaasApiClient, MaasApiError } = require("../../maas/MaasApiClient.ts");
...
class MockMaasApiClient {
   ...
   */
  async get(endpoint: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    if (endpoint === "/machines" || endpoint === "/machines/") {
      ...
      return this.listSubnets(params);
   ...
   */
  async post(endpoint: string, params: Record<string, any>, signal?: AbortSignal): Promise<any> {
    // Check if the request was aborted
    if (signal?.aborted) {
      throw new MaasApiError("Request aborted by client", 0);
    }
   ...
   */
  async put(endpoint: string, params: Record<string, any>, signal?: AbortSignal): Promise<any> {
   ...
   */
  async delete(endpoint: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
   ...
   */
  private _listMachines(params?: Record<string, any>): any[] {
    ...
    if (params) {
      ...
      if (params.tag_names) {
        ...
        filteredMachines = filteredMachines.filter(m => 
          tagNames.some((tag: string) => m.tags.includes(tag)));
      }
   ...
   */
  private _getMachine(systemId: string): any {
   ...
   */
  private listSubnets(params?: Record<string, any>): any[] {
   ...
   */
  private getSubnet(id: number): any {
   ...
   */
  private _createTag(params: Record<string, any>): any {
   ...
   */
  async listMachines(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    return this.get('/machines/', params, signal);
  }

  async getMachine(systemId: string, signal?: AbortSignal): Promise<any> {
    return this.get(`/machines/${systemId}/`, undefined, signal);
  }

  async createTag(name: string, comment?: string, definition?: string, signal?: AbortSignal): Promise<any> {
...
class MockMcpClient {
   ...
   */
  async callTool(toolName: string, params: any, signal?: AbortSignal): Promise<any> {
