...
import { MaasApiClient, MaasApiClientOptions } from '../../../maas/MaasApiClient';
import { MockMaasApiClient } from '../../mocks/mockMaasApiClient';
import {
    machineCollectionUri,
    tagCollectionUri,
    subnetCollectionUri,
    zoneCollectionUri,
    deviceCollectionUri, // Standalone devices
    domainCollectionUri,
} from '../../../mcp_resources/schemas/uriPatterns';
import { CacheManager } from '../../../mcp_resources/cache/cacheManager';
import { LruCacheStrategy } from '../../../mcp_resources/cache/lruCacheStrategy';
import { CacheOptions } from '../../../mcp_resources/cache/interfaces';
import { Logger } from '../../../utils/logger';
import { AuditLogger } from '../../../utils/auditLogger';
import { McpDataResponse, McpError } from '../../../types/modelcontextprotocol';

// Helper function to construct detail URIs, as uriPatterns.ts exports patterns not functions
const machineDetailUriFn = (systemId: string | number) => `maas://machines/${String(systemId)}`;
const tagDetailUriFn = (tagName: string | number) => `maas://tags/${String(tagName)}`;
const subnetDetailUriFn = (subnetId: string | number) => `maas://subnets/${String(subnetId)}`;
const zoneDetailUriFn = (zoneName: string | number) => `maas://zones/${String(zoneName)}`;
const deviceDetailUriFn = (deviceId: string | number) => `maas://devices/${String(deviceId)}`;
const domainDetailUriFn = (domainId: string | number) => `maas://domains/${String(domainId)}`;
...
describe('MCP Resource Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        mockMaasClient = new MockMaasApiClient({
            ...
            logger: mockLogger,
        } as MaasApiClientOptions);

        cacheManager = new CacheManager(new LruCacheStrategy({ maxSize: 100 }), {
            enabled: true,
    ...
    const testResourceCollection = (
        resourceName: string,
        collectionUri: string,
        mockClientMethod: keyof MockMaasApiClient,
        sampleData: any[],
        // Changed to support multiple/complex filter examples
        exampleFilterParams?: Record<string, string | number | boolean>,
        // Added to help test sorting more thoroughly
        sortableFields?: string[]
    ) => {
        describe(`GET Collection (${resourceName}) - Edge Cases and Core Functionality`, () => { // Updated describe title
            it(`should return a list of ${resourceName.toLowerCase()}`, async () => {
                (mockMaasClient[mockClientMethod] as jest.Mock).mockResolvedValue(sampleData.slice(0, 2));
            ...
            it('should handle pagination parameters (limit, offset)', async () => {
                (mockMaasClient[mockClientMethod] as jest.Mock).mockResolvedValue([sampleData[0]]);
            ...
            it('should handle sorting parameters (sort_by, order)', async () => {
                (mockMaasClient[mockClientMethod] as jest.Mock).mockResolvedValue(sampleData);
            ...
            it('should handle empty collections returned by MAAS', async () => {
                (mockMaasClient[mockClientMethod] as jest.Mock).mockResolvedValue([]);
                const response = await request(app)
                    .get(`/mcp/${serverName}/resources/${encodeURIComponent(collectionUri)}`)
                    .expect(200);
            ...
            it('should handle pagination with limit=1', async () => {
                (mockMaasClient[mockClientMethod] as jest.Mock).mockResolvedValue(sampleData.length > 0 ? [sampleData[0]] : []);
            ...
            it('should handle pagination with offset greater than total items', async () => {
                (mockMaasClient[mockClientMethod] as jest.Mock).mockResolvedValue([]); // MAAS would return empty
            ...
            if (exampleFilterParams && Object.keys(exampleFilterParams).length > 0) {
                it('should handle complex filtering parameters', async () => {
                    (mockMaasClient[mockClientMethod] as jest.Mock).mockResolvedValue([sampleData[0]]); // Assume first item matches
                    ...
                    const expectedMockParams: Record<string, any> = {};
                    for (const key in exampleFilterParams) {
            ...
            describe('Invalid Query Parameters', () => {
                ...
                invalidCases.forEach(({ param, expectedMsg, field }) => {
                    it(`should return 400 for invalid parameter: ${param}`, async () => {
                        ...
                        (mockMaasClient[mockClientMethod] as jest.Mock).mockClear();
                        (mockMaasClient[mockClientMethod] as jest.Mock).mockResolvedValue(sampleData); // Default mock if it were to be called
                        ...
                        const error = response.body.error as McpError;
            ...
            it('should use cache for identical subsequent requests', async () => {
                (mockMaasClient[mockClientMethod] as jest.Mock).mockResolvedValue(sampleData.slice(0,1));
                await request(app).get(`/mcp/${serverName}/resources/${encodeURIComponent(collectionUri)}`).expect(200);
                expect(mockMaasClient[mockClientMethod]).toHaveBeenCalledTimes(1);

                const cachedResponse = await request(app).get(`/mcp/${serverName}/resources/${encodeURIComponent(collectionUri)}`).expect(200);
            ...
            it('should not use cache for different requests', async () => {
                (mockMaasClient[mockClientMethod] as jest.Mock)
                ...
                await request(app).get(`/mcp/${serverName}/resources/${encodeURIComponent(differentUri)}`).expect(200);
            ...
            errorScenarios.forEach(scenario => {
                it(`should handle MAAS API ${scenario.statusCode} error`, async () => {
                    (mockMaasClient[mockClientMethod] as jest.Mock).mockRejectedValue({ isMaasApiError: true, response: { status: scenario.statusCode, data: scenario.maasMessage } });
    ...
    const testResourceDetail = (
        resourceName: string,
        detailUriFn: (id: string | number) => string,
        mockClientMethod: keyof MockMaasApiClient,
        ...
        describe(`GET Detail (${resourceName})`, () => {
            ...
            it('should use cache for identical subsequent detail requests', async () => {
                (mockMaasClient[mockClientMethod] as jest.Mock).mockResolvedValue(sampleData);
                await request(app).get(`/mcp/${serverName}/resources/${encodeURIComponent(detailUri)}`).expect(200);
                expect(mockMaasClient[mockClientMethod]).toHaveBeenCalledTimes(1);

                const cachedResponse = await request(app).get(`/mcp/${serverName}/resources/${encodeURIComponent(detailUri)}`).expect(200);
    ...
    describe('Machines Resource', () => {
        testResourceCollection(
            ...
            machineCollectionUri,
            'getMachines',
            [{ system_id: 'm1', hostname: 'host1', status: 6 }, { system_id: 'm2', hostname: 'host2', status: 4 }, { system_id: 'm3', hostname: 'host3', status: 6 }],
            { hostname: 'host1', status: 6 }, // Example complex filter params
        ...
        testResourceDetail(
            ...
            'getMachineDetails',
            'm1',
            { system_id: 'm1', hostname: 'host1', osystem: 'ubuntu' }
        );
    });
    ...
    describe('Tags Resource', () => {
        testResourceCollection(
            ...
            tagCollectionUri,
            'getTags',
            [{ name: 'tag1', definition: 'def1', kernel_opt: 'quiet' }, { name: 'tag2', definition: 'def2', kernel_opt: '' }, { name: 'tag3', definition: 'def3', kernel_opt: 'splash' }],
            { name: 'tag1', 'kernel_opt': 'quiet'}, // Tags can be filtered by name, definition, kernel_opt
            ['name', 'definition', 'kernel_opt']
        );
        testResourceDetail(
            'Tag',
            tagDetailUriFn,
            'getTag', // MockMaasApiClient.getTag(name)
            'tag1',
            { name: 'tag1', definition: 'def1', machines: [] }
        );
    });
    ...
    describe('Subnets Resource', () => {
        testResourceCollection(
            ...
            subnetCollectionUri,
            'getSubnets',
            [{ id: 1, name: 'subnet1', cidr: '192.168.1.0/24', fabric: 'fabric-0' }, { id: 2, name: 'subnet2', cidr: '10.0.0.0/8', fabric: 'fabric-1' }],
            { name: 'subnet1', fabric: 'fabric-0' },
            ['name', 'cidr', 'fabric', 'vlan']
        );
        testResourceDetail(
            'Subnet',
            subnetDetailUriFn,
            'getSubnet', // MockMaasApiClient.getSubnet(id)
            1,
            { id: 1, name: 'subnet1', cidr: '192.168.1.0/24', vlan: { vid: 10 } }
        );
    });
    ...
    describe('Zones Resource', () => {
        testResourceCollection(
            ...
            zoneCollectionUri,
            'getZones',
            [{ name: 'zone1', description: 'desc1' }, { name: 'zone2', description: 'desc2', id: 20 }],
            { name: 'zone1' }, // Zones can be filtered by name
            ['name', 'description']
        );
        testResourceDetail(
            'Zone',
            zoneDetailUriFn,
            'getZone', // MockMaasApiClient.getZone(name)
            'zone1',
            { name: 'zone1', description: 'desc1', resource_pools: [] }
        );
    });
    ...
    describe('Devices Resource (Standalone)', () => {
        testResourceCollection(
            'Devices',
            deviceCollectionUri,
            'getDevices', // MockMaasApiClient.getDevices() - for standalone
            [{ id: 'dev1', name: 'eth0', mac_address: 'AA:BB:CC:DD:EE:FF', type: 'physical' }, { id: 'dev2', name: 'wlan0', mac_address: '11:22:33:44:55:66', type: 'physical' }],
            { name: 'eth0', type: 'physical' }, // Devices can be filtered by name, id, mac_address, hostname, type
            ['name', 'id', 'mac_address', 'hostname', 'type']
        );
        testResourceDetail(
            'Device',
            deviceDetailUriFn,
            'getDevice', // MockMaasApiClient.getDevice(id) - for standalone
            'dev1',
            { id: 'dev1', name: 'eth0', mac_address: 'AA:BB:CC:DD:EE:FF', type: 'physical' }
        );
    });
    ...
    describe('Domains Resource', () => {
        testResourceCollection(
            ...
            domainCollectionUri,
            'getDomains',
            [{ id: 1, name: 'example.com', authoritative: true }, { id: 2, name: 'internal.local', authoritative: false, ttl: 600 }],
            { name: 'example.com', authoritative: true }, // Domains can be filtered by name, authoritative
            ['name', 'authoritative', 'ttl']
        );
        testResourceDetail(
            'Domain',
            domainDetailUriFn,
            'getDomain', // MockMaasApiClient.getDomain(id)
            1,
            { id: 1, name: 'example.com', authoritative: true, ttl: 3600 }
        );
    });
    ...
    describe('BaseResourceHandler Cache Integration', () => {
        ...
        let originalCacheOptions: CacheOptions | undefined;
        let machineResourceHandler: any;
        ...
        beforeEach(() => {
            ...
            (mockMaasClient.getMachineDetails as jest.Mock).mockResolvedValue(sampleMachineData);
        ...
        describe('CacheManager Interaction', () => {
            it('should call CacheManager.get() before MAAS API call on cache miss', async () => {
                ...
                cacheManager.clear();
                getSpy.mockClear();
                (mockMaasClient.getMachineDetails as jest.Mock).mockClear();
                ...
                const maasCallOrder = (mockMaasClient.getMachineDetails as jest.Mock).mock.invocationCallOrder[0];
    ...
    describe('XML Content Negotiation', () => {
        ...
        const generateExpectedXml = (json: any): string => {
            let xml = '<root>';
        ...
        beforeEach(() => {
            ...
            (mockMaasClient.getMachineDetails as jest.Mock).mockReset();
            (mockMaasClient.getMachineDetails as jest.Mock).mockResolvedValue(sampleJsonResponse);
        });
        ...
        describe('1. XML Response Formatting & Structure', () => {
            ...
            it('should handle simple JSON objects', async () => {
                ...
                (mockMaasClient.getMachineDetails as jest.Mock).mockResolvedValue(simpleJson);
                const response = await request(app)
                    .get(`/mcp/${serverName}/resources/${encodeURIComponent(testUri + '?format=xml')}`)
                    .expect(200);
                expect(response.headers['content-type']).toMatch(/application\/xml/);
            ...
            it('should handle nested JSON objects', async () => {
                ...
                (mockMaasClient.getMachineDetails as jest.Mock).mockResolvedValue(nestedJson);
                const response = await request(app)
                    .get(`/mcp/${serverName}/resources/${encodeURIComponent(testUri + '?format=xml')}`)
                    .expect(200);
                expect(response.headers['content-type']).toMatch(/application\/xml/);
            ...
            it('should handle arrays in JSON', async () => {
                ...
                (mockMaasClient.getMachineDetails as jest.Mock).mockResolvedValue(arrayJson);
                const response = await request(app)
                    .get(`/mcp/${serverName}/resources/${encodeURIComponent(testUri + '?format=xml')}`)
                    .expect(200);
                expect(response.headers['content-type']).toMatch(/application\/xml/);
            ...
            it('should handle empty JSON objects', async () => {
                (mockMaasClient.getMachineDetails as jest.Mock).mockResolvedValue({});
                const response = await request(app)
                    .get(`/mcp/${serverName}/resources/${encodeURIComponent(testUri + '?format=xml')}`)
                    .expect(200);
                expect(response.headers['content-type']).toMatch(/application\/xml/);
            ...
            it('should handle empty arrays in JSON', async () => {
                const emptyArrayJson = { id: 1, items: [] };
                 (mockMaasClient.getMachineDetails as jest.Mock).mockResolvedValue(emptyArrayJson);
                const response = await request(app)
                    .get(`/mcp/${serverName}/resources/${encodeURIComponent(testUri + '?format=xml')}`)
                    .expect(200);
                expect(response.headers['content-type']).toMatch(/application\/xml/);
            ...
            it('should correctly escape special XML characters', async () => {
                ...
                (mockMaasClient.getMachineDetails as jest.Mock).mockResolvedValue(specialCharsJson);
                const response = await request(app)
                    .get(`/mcp/${serverName}/resources/${encodeURIComponent(testUri + '?format=xml')}`)
                    .expect(200);
                expect(response.headers['content-type']).toMatch(/application\/xml/);
        ...
        describe('3. Request Content Type Tests', () => {
            it('should accept JSON content type for POST/PUT requests', async () => {
                ...
                (mockMaasClient.createMachine as jest.Mock) = jest.fn().mockResolvedValue({
                    ...testData,
        ...
        describe('6. Edge Case Tests', () => {
            it('should handle empty response bodies correctly', async () => {
                ...
                (mockMaasClient.deleteMachine as jest.Mock) = jest.fn().mockResolvedValue(undefined);
            ...
            it('should handle large payloads', async () => {
                ...
                // Mock the MAAS client method
                (mockMaasClient.bulkOperation as jest.Mock) = jest.fn().mockResolvedValue({
                    success: true,
                    processed: 1000
                });
            ...
            it('should handle nested structures correctly in XML', async () => {
                ...
                (mockMaasClient.getMachineDetails as jest.Mock).mockResolvedValue(deeplyNestedJson);
        ...
        describe('7. XML Conversion Fallback', () => {
            ...
            it('should gracefully handle potential XML conversion issues (e.g., by returning a server error in JSON)', async () => {
                ...
                (mockMaasClient.getMachineDetails as jest.Mock).mockResolvedValue(problematicData);
