import { registerListSubnetsTool } from '../../mcp_tools/listSubnets.js';
...
describe('listSubnets', () => {
  ...
  beforeEach(() => {
    ...
    // Register the tool
    registerListSubnetsTool(mockServer, mockMaasClient);
  ...
  it('should handle successful subnet listing with no filters', async () => {
    ...
    expect(mockSendNotification.mock.calls.some(call =>
      ...
      call[0].params.message === 'Starting subnet list retrieval...'
    )).toBe(true);
    ...
    expect(mockSendNotification.mock.calls.some(call =>
      ...
      call[0].params.message === 'Subnet list retrieved successfully.'
    )).toBe(true);
