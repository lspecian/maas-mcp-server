import { registerTagManagementTools } from '../../mcp_tools/tagManagement.js';
...
describe('tagManagement', () => {
  ...
  let mockCreateTagCallback: jest.Mock;
  let mockUpdateTagNodesCallback: jest.Mock;
  let mockDeleteTagCallback: jest.Mock;
  ...
  beforeEach(() => {
    ...
    // Register the tools
    registerTagManagementTools(mockServer, mockMaasClient);
    
    // Capture the tool callbacks
    // The first call is for maas_create_tag
    mockCreateTagCallback = mockServer.tool.mock.calls[0][3] as unknown as jest.Mock;
    // The second call is for maas_update_tag_nodes
    mockUpdateTagNodesCallback = mockServer.tool.mock.calls[1][3] as unknown as jest.Mock;
    // The third call is for maas_delete_tag
    mockDeleteTagCallback = mockServer.tool.mock.calls[2][3] as unknown as jest.Mock;
  });
  ...
  describe('maas_update_tag_nodes', () => {
    ...
    it('should handle successful tag nodes update with add and remove', async () => {
      ...
      expect(mockSendNotification.mock.calls.some(call =>
        ...
        call[0].params.progressToken === 'test-token' &&
        call[0].params.progress === 0 &&
        call[0].params.message === 'Starting tag node update...'
      )).toBe(true);
      ...
      expect(mockSendNotification.mock.calls.some(call =>
        ...
        call[0].params.progressToken === 'test-token' &&
        call[0].params.progress === 100 &&
        call[0].params.message === 'Tag nodes updated successfully.'
      )).toBe(true);
  ...
  describe('maas_delete_tag', () => {
    ...
    it('should handle successful tag deletion', async () => {
      ...
      expect(mockMaasClient.delete).toHaveBeenCalledWith(
        '/tags/test-tag',
        undefined,
        expect.any(AbortSignal)
      );
      ...
      expect(mockSendNotification.mock.calls.some(call =>
        ...
        call[0].params.progressToken === 'test-token' &&
        call[0].params.progress === 0 &&
        call[0].params.message === "Starting deletion of tag 'test-tag'..."
      )).toBe(true);
      ...
      expect(mockSendNotification.mock.calls.some(call =>
        ...
        call[0].params.progressToken === 'test-token' &&
        call[0].params.progress === 100 &&
        call[0].params.message === 'Tag deleted successfully.'
      )).toBe(true);
