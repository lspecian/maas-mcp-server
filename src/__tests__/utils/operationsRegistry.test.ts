...
describe('operationsRegistry', () => {
  ...
  describe('OperationsRegistry class', () => {
    ...
    it('should create a new registry with custom cleanup config', () => {
      ...
      const registry = new OperationsRegistry(customConfig);
      expect(registry.size).toBe(0);
    });
    ...
    it('should abort an operation', () => {
      ...
      const operation = registry.getOperation('test-token');
      expect(operation?.status).toBe(OperationStatus.ABORTED);
    ...
    it('should query operations by status', () => {
      const registry = new OperationsRegistry();
      registry.registerOperation('token1', 'testOperation', { initialStatus: OperationStatus.PENDING });
      registry.registerOperation('token2', 'testOperation', { initialStatus: OperationStatus.RUNNING });
      registry.registerOperation('token3', 'testOperation', { initialStatus: OperationStatus.COMPLETED });
      
      const pendingOps = registry.queryOperations({ status: OperationStatus.PENDING });
      const runningOps = registry.queryOperations({ status: OperationStatus.RUNNING });
    ...
    it('should clean up stale operations', () => {
      ...
      registry.registerOperation('token2', 'testOperation', { initialStatus: OperationStatus.RUNNING });
      registry.registerOperation('token3', 'testOperation', { initialStatus: OperationStatus.PENDING });
  ...
  describe('Singleton functions', () => {
    it('should register an operation using the singleton', () => {
      const operation = registerOperation('test-token', 'testOperation');
      ...
      expect(getOperationsRegistry().size).toBe(1);
    });

    it('should update an operation using the singleton', () => {
      registerOperation('test-token', 'testOperation');
      
      const updated = updateOperation('test-token', {
        status: OperationStatus.RUNNING,
        progress: 50
      });
    ...
    it('should get an operation using the singleton', () => {
      ...
      const operation = getOperation('test-token');
    ...
    it('should remove an operation using the singleton', () => {
      ...
      const removed = removeOperation('test-token');
      
      expect(removed).toBe(true);
      expect(getOperationsRegistry().size).toBe(0);
    });
    ...
    it('should abort an operation using the singleton', () => {
      ...
      const aborted = abortOperation('test-token');
    ...
    it('should query operations using the singleton', () => {
      registerOperation('token1', 'typeA', { initialStatus: OperationStatus.PENDING });
      registerOperation('token2', 'typeB', { initialStatus: OperationStatus.RUNNING });
      
      const pendingOps = queryOperations({ status: OperationStatus.PENDING });
      const typeAOps = queryOperations({ operationType: 'typeA' });
    ...
    it('should get active operations using the singleton', () => {
      registerOperation('token1', 'testOperation', { initialStatus: OperationStatus.PENDING });
      registerOperation('token2', 'testOperation', { initialStatus: OperationStatus.RUNNING });
      registerOperation('token3', 'testOperation', { initialStatus: OperationStatus.COMPLETED });
    ...
    it('should get operations by type using the singleton', () => {
      registerOperation('token1', 'typeA');
      registerOperation('token2', 'typeB');
      registerOperation('token3', 'typeA');
      
      const typeAOps = getOperationsByType('typeA');
      
      expect(typeAOps.length).toBe(2);
      expect(typeAOps.map(op => op.progressToken)).toEqual(expect.arrayContaining(['token1', 'token3']));
    });

    it('should clean up stale operations using the singleton', () => {
      // Override the default cleanup config for testing
      const registry = getOperationsRegistry();
      registry.clear();
      
      // Register operations
      const time1 = 1000;
      jest.spyOn(Date, 'now').mockReturnValue(time1);
      registerOperation('token1', 'testOperation', { initialStatus: OperationStatus.COMPLETED });
      
      // Advance time beyond the default maxCompletedAge
      jest.spyOn(Date, 'now').mockReturnValue(time1 + DEFAULT_CLEANUP_CONFIG.maxCompletedAge! + 1000);
      
      cleanupStaleOperations();
      
      // Operation should be cleaned up
      expect(getOperation('token1')).toBeUndefined();
    });
  });
  ...
  describe('Concurrent access', () => {
    it('should handle concurrent operations safely', () => {
      ...
      const operations = Array.from({ length: 10 }, (_, i) => 
        registerOperation(`token-${i}`, 'concurrentTest')
      );
      
      // Update operations concurrently
      const updatePromises = operations.map((op, i) => 
        updateOperation(`token-${i}`, { progress: i * 10 })
      );
      ...
      expect(getOperationsRegistry().size).toBe(10);
      
      // Verify each operation has the correct progress
      operations.forEach((_, i) => {
        const op = getOperation(`token-${i}`);
  ...
  describe('Integration with AbortSignal', () => {
    it('should abort the operation when the parent signal is aborted', async () => {
      const controller = new AbortController();
