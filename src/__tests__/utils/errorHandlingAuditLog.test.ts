...
describe('Error Handling with Audit Logging Integration', () => {
  ...
  describe('Request ID Generation', () => {
    it('should generate a unique request ID for each error handling function call', () => {
      ...
      try {
        ...
        extractAndValidateParams('test://uri', 'test://pattern', z.any(), 'TestResource');
      } catch (error) {
        // Expected error
      }

      try {
        validateResourceData({ invalid: 'data' }, z.object({ valid: z.string() }), 'TestResource');
      } catch (error) {
        // Expected error
      }

      try {
        handleResourceFetchError(new Error('Test error'), 'TestResource', '123');
      } catch (error) {
        // Expected error
      }
  ...
  describe('ZodError Handling in Parameter Validation', () => {
    it('should log ZodError to audit logger with request ID in extractAndValidateParams', () => {
      ...
      (mockExtractParamsFromUri as jest.Mock).mockReturnValue({ id: '' }); // Will fail validation
      
      try {
        extractAndValidateParams('test://resource', 'test://pattern', mockSchema, 'TestResource');
      ...
      expect(auditLogger.logResourceAccessFailure).toHaveBeenCalledWith(
        'TestResource',
        undefined,
        'validate_params',
        'test-request-id',
        expect.any(MaasApiError),
        undefined,
        undefined,
      ...
      const maasError = (auditLogger.logResourceAccessFailure as jest.Mock).mock.calls[0][4];
      expect(maasError).toBeInstanceOf(MaasApiError);
    ...
    it('should log ZodError to audit logger with request ID in validateResourceData', () => {
      ...
      try {
        validateResourceData(invalidData, mockSchema, 'TestResource', resourceId);
        fail('Should have thrown an error');
      } catch (error) {
        // Expected error
      }
  ...
  describe('MaasApiError Handling', () => {
    it('should log existing MaasApiError to audit logger in extractAndValidateParams', () => {
      ...
      try {
        extractAndValidateParams('test://resource', 'test://pattern', z.any(), 'TestResource');
        fail('Should have thrown an error');
      } catch (error) {
        // Expected error
      }
    ...
    it('should log MaasApiError with 404 transformation in handleResourceFetchError', () => {
      ...
      try {
        handleResourceFetchError(originalError, 'TestResource', resourceId);
        fail('Should have thrown an error');
      } catch (error) {
        // Expected error
      }
  ...
  describe('Generic Error Handling', () => {
    ...
    it('should log non-ZodErrors in validateResourceData', () => {
      ...
      try {
        validateResourceData({}, mockFailingSchema, 'TestResource', resourceId);
        fail('Should have thrown an error');
      } catch (error) {
        // Expected error
      }
    ...
    it('should transform and log generic errors in handleResourceFetchError', () => {
      ...
      try {
        handleResourceFetchError(genericError, 'TestResource', resourceId, context);
        fail('Should have thrown an error');
      } catch (error) {
        // Expected error
      }
  ...
  describe('Network Error Handling', () => {
    it('should transform and log ECONNREFUSED errors', () => {
      ...
      try {
        handleResourceFetchError(networkError, 'TestResource', resourceId, context);
        fail('Should have thrown an error');
      } catch (error) {
        // Expected error
      }
  ...
  describe('Timeout Error Handling', () => {
    it('should transform and log ETIMEDOUT errors', () => {
      ...
      try {
        handleResourceFetchError(timeoutError, 'TestResource', resourceId, context);
        fail('Should have thrown an error');
      } catch (error) {
        // Expected error
      }
  ...
  describe('AbortError Handling', () => {
    it('should transform and log AbortError', () => {
      ...
      try {
        handleResourceFetchError(abortError, 'TestResource', resourceId, context);
        fail('Should have thrown an error');
      } catch (error) {
        // Expected error
      }
  ...
  describe('Error Details Completeness', () => {
    it('should include complete error details in audit logs', () => {
      ...
      (originalError as any).code = 'TEST_ERROR_CODE';
      (originalError as any).details = { additionalInfo: 'test info' };
      ...
      try {
        handleResourceFetchError(originalError, 'TestResource', resourceId, context);
        fail('Should have thrown an error');
      } catch (error) {
        // Expected error
      }
      ...
      const contextParam = (auditLogger.logResourceAccessFailure as jest.Mock).mock.calls[0][7];
