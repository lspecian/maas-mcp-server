...
// 1. Imports
import { ExampleClass } from '../../example/ExampleModule';
...
describe('ExampleModule', () => {
  ...
  describe('constructor', () => {
    test('should initialize with default options when no options are provided', () => {
      // Arrange
      const defaultInstance = new ExampleClass();
    ...
    test('should throw an error when invalid options are provided', () => {
      ...
      // Act & Assert
      expect(() => createExampleInstance(invalidOptions)).toThrow(SomeError);
  ...
  describe('processData', () => {
    ...
    test('should throw an error when processing invalid data', async () => {
      ...
      // Act & Assert
      await expect(exampleInstance.processData(invalidData)).rejects.toThrow(SomeError);
