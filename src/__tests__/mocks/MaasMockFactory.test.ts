import { MaasMockFactory } from './MaasMockFactory';
import { MaasApiClient } from '../../maas/MaasApiClient';
import { MaasApiError } from '../../types';
import { mockMachines, mockSubnets } from './mockMaasApiClient'; // Import some mock data

describe('MaasMockFactory', () => {
  it('should create a default client that returns mockMachines', async () => {
    const client = MaasMockFactory.defaultClient();
  ...
  it('should create an empty client that returns an empty array', async () => {
    const client = MaasMockFactory.emptyClient();
  ...
  it('should create a slow client with specified delay', async () => {
    ...
    const client = MaasMockFactory.slowClient(delay);
  ...
  it('should create a high-latency client with specified delay and jitter', async () => {
    const delay = 20;
    const jitter = 10;
    const client = MaasMockFactory.highLatencyClient(delay, jitter);
    const startTime = Date.now();
    await client.get('/machines/');
    const endTime = Date.now();
    expect(endTime - startTime).toBeGreaterThanOrEqual(delay);
  ...
  it('should create a notFound client that throws a 404 error', async () => {
    const client = MaasMockFactory.notFoundClient();
    try {
  ...
  it('should create a serverError client that throws a 500 error', async () => {
    const client = MaasMockFactory.serverErrorClient();
    try {
  ...
  it('should create an unauthorized client that throws a 401 error', async () => {
    const client = MaasMockFactory.unauthorizedClient();
    try {
      await client.get('/any-endpoint/');
    } catch (e: any) {
      expect(e).toBeInstanceOf(MaasApiError);
  ...
  it('should create a forbidden client that throws a 403 error', async () => {
    const client = MaasMockFactory.forbiddenClient();
    try {
      await client.get('/any-endpoint/');
    } catch (e: any) {
      expect(e).toBeInstanceOf(MaasApiError);
  ...
  it('should create a validationError client that throws a 422 error', async () => {
    const client = MaasMockFactory.validationErrorClient();
    try {
      await client.get('/any-endpoint/');
    } catch (e: any) {
      expect(e).toBeInstanceOf(MaasApiError);
  ...
  it('should create a paginated client', async () => {
    ...
    const factory = new MaasMockFactory()
      .withSuccessResponse(mockMachines)
    ...
    expect(response.total).toBe(mockMachines.length);
  ...
  it('should create a flaky client that can succeed or fail', async () => {
    ...
    const client = MaasMockFactory.flakyClient(0.5, 10); // 50% error rate, 10ms delay
    let success = false;
    let errors = 0;
    for (let i = 0; i < 10; i++) {
      try {
  ...
  it('should create a rateLimited client', async () => {
    const threshold = 2;
    const client = MaasMockFactory.rateLimitedClient(threshold, 10);
  ...
  it('should create a realisticNetwork client with combined effects', async () => {
    const client = MaasMockFactory.realisticNetworkClient();
  ...
  it('should allow chaining of configurations', async () => {
    ...
    const client = factory
      .withSuccessResponse(mockSubnets)
      .withNetworkDelay(10)
      .withPagination(1)
      .create();
    ...
    expect(response.items[0]).toEqual(mockSubnets[0]);
    expect(response.total).toBe(mockSubnets.length);
  });
  
  it('should use default successResponse if none provided and no error', async () => {
    const client = new MaasMockFactory().create();
    const response = await client.get('/some-endpoint/');
    expect(response).toEqual(mockMachines); // Default is mockMachines
  });

  it('should create a client with variable latency', async () => {
    const client = MaasMockFactory.variableLatencyClient(20, 10); // 20ms/KB, 10ms base
    const startTime = Date.now();
  ...
  it('should create a client with limited bandwidth', async () => {
    const client = MaasMockFactory.limitedBandwidthClient(50); // 50 KB/s
    const startTime = Date.now();
  ...
  it('should create a client with connection drops', async () => {
    const client = MaasMockFactory.connectionDropsClient(1.0); // 100% drop probability
    try {
      await client.get('/machines/');
    } catch (e: any) {
      expect(e).toBeInstanceOf(MaasApiError);
  ...
  it('should create a client with progressive degradation', async () => {
    const client = MaasMockFactory.progressiveDegradationClient(10, 1.2);
    const times = [];
    for (let i = 0; i < 3; i++) {
  ...
  it('should create a client with geographic latency', async () => {
    const client = MaasMockFactory.geographicLatencyClient('global'); // Default global is 300ms
    const startTime = Date.now();
    ...
    const factoryWithOptions = new MaasMockFactory().withGeographicLatency('global');
    const internalOptions = (factoryWithOptions as any).options; // Access private for test
  ...
  it('should allow overriding options with withOptions', async () => {
    const client = new MaasMockFactory()
  ...
  it('should correctly use specificErrors configuration', async () => {
    ...
    expect(successResponse).toEqual(mockMachines); // Default success
  });
});