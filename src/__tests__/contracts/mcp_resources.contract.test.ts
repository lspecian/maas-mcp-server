...
import {
  ...
  DomainsSchema,
} from '../../mcp_resources/schemas/index.js';
import { ZodSchema } from 'zod';
import { parseStringPromise as xml2jsParse } from 'xml2js';
...
const MockedMaasApiClient = MaasApiClient as jest.MockedClass<typeof MaasApiClient>;
...
describe('MCP Resource API Contract Tests', () => {
  ...
  let apiClientMock: jest.Mocked<MaasApiClient>;
  ...
  const verifyErrorFormat = (response: any) => {
  ...
  const testQueryParameter = async (
    ...
    (apiClientMock.get as jest.Mock).mockImplementationOnce(async (endpoint, params) => {
  ...
  const testMultipleQueryParameters = async (
    app: ExpressApplication,
    uri: string,
  ...
  beforeAll(async () => {
    apiClientMock = new MockedMaasApiClient({
      ...
      apiKey: 'fake-key',
    }) as jest.Mocked<MaasApiClient>;

    auditLoggerMock = new MockedAuditLogger() as jest.Mocked<AuditLogger>;
    (auditLoggerMock.log as jest.Mock).mockImplementation(() => Promise.resolve());
    ...
    (apiClientMock.get as jest.Mock).mockImplementation(async (endpoint: string) => {
      if (endpoint.includes('/machines/machine-1')) return { data: mockMachineDetailsData, headers: {} };
      if (endpoint.includes('/machines')) return { data: mockMachinesData, headers: {} };
      if (endpoint.includes('/tags/tag-1')) return { data: mockTagDetailsData, headers: {} };
      if (endpoint.includes('/tags')) return { data: mockTagsData, headers: {} };
      if (endpoint.includes('/subnets/subnet-1')) return { data: mockSubnetDetailsData, headers: {} };
      if (endpoint.includes('/subnets')) return { data: mockSubnetsData, headers: {} };
      if (endpoint.includes('/zones/zone-1')) return { data: mockZoneDetailsData, headers: {} };
      if (endpoint.includes('/zones')) return { data: mockZonesData, headers: {} };
      if (endpoint.includes('/devices/device-1')) return { data: mockDeviceDetailsData, headers: {} };
      if (endpoint.includes('/devices')) return { data: mockDevicesData, headers: {} };
      if (endpoint.includes('/domains/domain-1')) return { data: mockDomainDetailsData, headers: {} };
      if (endpoint.includes('/domains')) return { data: mockDomainsData, headers: {} };
      return { data: {}, headers: {} };
    });
  ...
  beforeEach(() => {
    ...
    (apiClientMock.get as jest.Mock).mockImplementation(async (endpoint: string) => {
      if (endpoint.startsWith('machines/machine-1')) return { data: mockMachineDetailsData, headers: { 'cache-control': 'public, max-age=60' } };
      if (endpoint.startsWith('machines')) return { data: mockMachinesData, headers: { 'cache-control': 'public, max-age=60' } };
      if (endpoint.startsWith('tags/tag-1')) return { data: mockTagDetailsData, headers: { 'cache-control': 'public, max-age=300' } };
      if (endpoint.startsWith('tags')) return { data: mockTagsData, headers: { 'cache-control': 'public, max-age=300' } };
      if (endpoint.startsWith('subnets/subnet-1')) return { data: mockSubnetDetailsData, headers: { 'cache-control': 'public, max-age=3600' } };
      if (endpoint.startsWith('subnets')) return { data: mockSubnetsData, headers: { 'cache-control': 'public, max-age=3600' } };
      if (endpoint.startsWith('zones/zone-1')) return { data: mockZoneDetailsData, headers: { 'cache-control': 'public, max-age=86400' } };
      if (endpoint.startsWith('zones')) return { data: mockZonesData, headers: { 'cache-control': 'public, max-age=86400' } };
      if (endpoint.startsWith('devices/device-1')) return { data: mockDeviceDetailsData, headers: { 'cache-control': 'public, max-age=60' } };
      if (endpoint.startsWith('devices')) return { data: mockDevicesData, headers: { 'cache-control': 'public, max-age=60' } };
      if (endpoint.startsWith('domains/domain-1')) return { data: mockDomainDetailsData, headers: { 'cache-control': 'public, max-age=3600' } };
      if (endpoint.startsWith('domains')) return { data: mockDomainsData, headers: { 'cache-control': 'public, max-age=3600' } };
      return { data: {}, headers: {} };
    });
    
    // Reset audit logger mock
    (auditLoggerMock.log as jest.Mock).mockClear();
  });

  interface TestResourceEndpointsOptions {
    resourceName: string;
    ...
    mockItemData: any;
    collectionSchema: ZodSchema<any>;
    ...
    queryParams?: {
      ...
      page?: string;
      limit?: string;
      // Additional resource-specific query parameters
      [key: string]: string | undefined;
    };
  }

  const testResourceEndpoints = ({
    resourceName,
    collectionUri,
    itemUriBase,
    itemId,
    ...
    describe(`${resourceName} Resources`, () => {
      ...
      describe(`GET ${collectionUri}`, () => {
        it('should return 200 OK with JSON data matching schema', async () => {
          (apiClientMock.get as jest.Mock).mockResolvedValueOnce({ data: mockCollectionData, headers: { 'content-type': 'application/json', 'cache-control': expectedCacheControl } });
          const response = await request(app)
            .get(collectionUri)
            .set('Accept', 'application/json');
        ...
        it('should return 200 OK with XML data matching schema (converted)', async () => {
          (apiClientMock.get as jest.Mock).mockResolvedValueOnce({ data: mockCollectionData, headers: { 'content-type': 'application/xml', 'cache-control': expectedCacheControl } });
          const response = await request(app)
            .get(collectionUri)
            .set('Accept', 'application/xml');
          expect(response.status).toBe(200);
          expect(response.headers['content-type']).toMatch(/application\/xml/);
          const xmlResponse = await xml2jsParse(response.text, { explicitArray: false, mergeAttrs: true });
        ...
        if (queryParams) {
          ...
          it('should handle boundary values for numeric parameters', async () => {
            ...
            (apiClientMock.get as jest.Mock).mockRejectedValueOnce({
              isAxiosError: true,
              response: {
                status: 400,
          ...
          it('should handle invalid query parameters appropriately', async () => {
            ...
            const unknownParamResponse = await request(app)
        ...
        it('should handle empty collections gracefully', async () => {
          (apiClientMock.get as jest.Mock).mockResolvedValueOnce({
            data: [],
      ...
      describe(`GET ${itemUri}`, () => {
        it('should return 200 OK with JSON data matching schema', async () => {
          (apiClientMock.get as jest.Mock).mockResolvedValueOnce({ data: mockItemData, headers: { 'content-type': 'application/json', 'cache-control': expectedCacheControl } });
          const response = await request(app)
            .get(itemUri)
            .set('Accept', 'application/json');
          expect(response.status).toBe(200);
        ...
        it('should return 200 OK with XML data matching schema (converted)', async () => {
          (apiClientMock.get as jest.Mock).mockResolvedValueOnce({ data: mockItemData, headers: { 'content-type': 'application/xml', 'cache-control': expectedCacheControl } });
          const response = await request(app)
            .get(itemUri)
            .set('Accept', 'application/xml');
          expect(response.status).toBe(200);
        ...
        it('should return 404 Not Found for non-existent item ID', async () => {
          (apiClientMock.get as jest.Mock).mockRejectedValueOnce({ isAxiosError: true, response: { status: 404, data: 'Not Found' } });
          const response = await request(app)
      ...
      describe('HTTP Status Code Mapping', () => {
        ...
        testCases.forEach(({ maasStatus, mcpStatus, description }) => {
          it(`should map MAAS ${maasStatus} to MCP ${mcpStatus} (${description}) for collection`, async () => {
            ...
            const response = await request(app).get(collectionUri).set('Accept', 'application/json');
          ...
          it(`should map MAAS ${maasStatus} to MCP ${mcpStatus} (${description}) for item`, async () => {
            ...
            const response = await request(app).get(itemUri).set('Accept', 'application/json');
  ...
  testResourceEndpoints({
    ...
    collectionSchema: MachinesSchema,
    itemSchema: MachineDetailsSchema,
  ...
  testResourceEndpoints({
    ...
    collectionSchema: TagsSchema,
    itemSchema: TagResourceSchema, // Assuming TagResourceSchema is for individual tag details
  ...
  testResourceEndpoints({
    ...
    collectionSchema: SubnetsSchema,
    itemSchema: SubnetResourceSchema,
  ...
  testResourceEndpoints({
    ...
    collectionSchema: ZonesSchema,
    itemSchema: ZoneResourceSchema,
  ...
  testResourceEndpoints({
    ...
    collectionSchema: DevicesSchema,
    itemSchema: DeviceResourceSchema,
    expectedCacheControl: 'public, max-age=60',
    queryParams: {
  ...
  testResourceEndpoints({
    ...
    collectionSchema: DomainsSchema,
    itemSchema: DomainResourceSchema,
    expectedCacheControl: 'public, max-age=3600',
    queryParams: {
      filter: 'name=test',
      sort: 'name',
