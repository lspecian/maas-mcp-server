...
import { MaasApiError } from '../../types/index.js'; // Corrected import path
import { ResourceTemplate, McpServer } from '@modelcontextprotocol/sdk/server/mcp.js';
import { MaasMachineSchema } from '../../mcp_resources/schemas/machineDetailsSchema.js';
import {
  machineDetailsTemplate,
  MACHINE_DETAILS_URI_PATTERN,
  registerMachineDetailsResource
} from '../../mcp_resources/machineDetails.js';
...
describe('Machine Details Resource', () => {
  ...
  let registeredHandler: ResourceHandler;

  // Setup before tests
  beforeAll(() => {
    // Register the resource and capture the handler
    registerMachineDetailsResource(mockServer as unknown as McpServer, mockMaasClient);
    
    // Extract the handler function that was registered
    registeredHandler = mockServer.resource.mock.calls[0][2] as ResourceHandler;
  });
  ...
  test('should handle specific HTTP error codes from MAAS API', async () => {
    ...
    mockMaasClient.get.mockRejectedValue(new MaasApiError(errorMessage));
  ...
  test('should handle schema validation errors', async () => {
    ...
    (MaasMachineSchema.parse as jest.Mock).mockImplementationOnce(() => {
  ...
  test('should handle unexpected server response formats', async () => {
    ...
    (MaasMachineSchema.parse as jest.Mock).mockReturnValueOnce(invalidResponse);
