...
describe('URI Pattern Extraction and Validation', () => {
  ...
  const TestQueryParamsSchema = z.object({
  ...
  describe('Parameter Extraction Edge Cases', () => {
    ...
    it('should handle URIs with multiple parameters', () => {
      ...
      const params = extractParamsFromUri(uri, multiParamPattern);
      expect(params).toEqual({
  ...
  describe('Parameter Validation', () => {
    it('should validate parameters against schema', () => {
      ...
      const result = extractAndValidateParams(
        uri,
        pattern,
        TestParamsSchema,
        'TestResource'
    ...
    it('should throw error for missing required parameters', () => {
      ...
      expect(() => {
        extractAndValidateParams(
          uri,
          pattern,
          TestParamsSchema.extend({ param_id: z.string().min(1) }),
          'TestResource'
        );
    ...
    it('should throw error for invalid parameter format', () => {
      ...
      expect(() => {
        extractAndValidateParams(
          uri,
          pattern,
          TestParamsSchema.extend({ param_id: z.string().regex(/^[a-z]+$/) }),
          'TestResource'
        );
      }).toThrow(MaasApiError);
    });
  ...
  describe('Resource Handler Integration', () => {
    it('should extract parameters from all resource handler URI patterns', () => {
      const testCases = [
        { uri: 'maas://machine/abc123/details', pattern: MACHINE_DETAILS_URI_PATTERN, expected: { system_id: 'abc123' } },
        { uri: 'maas://machines/list', pattern: MACHINES_LIST_URI_PATTERN, expected: {} },
        { uri: 'maas://tag/test-tag/details', pattern: TAG_DETAILS_URI_PATTERN, expected: { tag_name: 'test-tag' } },
        { uri: 'maas://tags/list', pattern: TAGS_LIST_URI_PATTERN, expected: {} },
        { uri: 'maas://tag/test-tag/machines', pattern: TAG_MACHINES_URI_PATTERN, expected: { tag_name: 'test-tag' } },
        { uri: 'maas://subnet/123/details', pattern: SUBNET_DETAILS_URI_PATTERN, expected: { subnet_id: '123' } },
        { uri: 'maas://subnets/list', pattern: SUBNETS_LIST_URI_PATTERN, expected: {} },
        { uri: 'maas://zone/default/details', pattern: ZONE_DETAILS_URI_PATTERN, expected: { zone_id: 'default' } },
        { uri: 'maas://zones/list', pattern: ZONES_LIST_URI_PATTERN, expected: {} },
        { uri: 'maas://device/dev123/details', pattern: DEVICE_DETAILS_URI_PATTERN, expected: { system_id: 'dev123' } },
        { uri: 'maas://devices/list', pattern: DEVICES_LIST_URI_PATTERN, expected: {} },
        { uri: 'maas://domain/example/details', pattern: DOMAIN_DETAILS_URI_PATTERN, expected: { domain_id: 'example' } },
        { uri: 'maas://domains/list', pattern: DOMAINS_LIST_URI_PATTERN, expected: {} },
      ];
  ...
  describe('Performance Considerations', () => {
    it('should handle large numbers of URI parameters efficiently', () => {
      ...
      const params = extractParamsFromUri(largeUri, largePattern);
