...
import { SubnetDetailsResourceHandler, SubnetsListResourceHandler, registerSubnetResources } from '../../mcp_resources/handlers/SubnetResourceHandler.js';
import { MaasSubnetSchema, SubnetCollectionQueryParamsSchema, GetSubnetParamsSchema } from '../../mcp_resources/schemas/index.js';
...
describe('Subnet Resource Handlers', () => {
  ...
  describe('SubnetDetailsResourceHandler', () => {
    let handler: SubnetDetailsResourceHandler;
    let registeredCallback: HandlerCallbackType;

    beforeEach(async () => {
      await import('../../mcp_resources/schemas/uriPatterns.js');
      handler = new SubnetDetailsResourceHandler(mockMcpServer, mockMaasApiClient);
    ...
    it('should register with correct parameters', async () => {
      ...
      expect(ResourceTemplate).toHaveBeenCalledWith(mockedUriPatterns.SUBNET_DETAILS_URI_PATTERN, { list: undefined });
      expect(mockMcpServer.resource).toHaveBeenCalledWith('maas_subnet_details', expect.any(ResourceTemplate), expect.any(Function));
    });
    ...
    it('should use cache if available for subnet details', async () => {
      ...
      expect(result.contents[0].text).toBe(JSON.stringify(mockSubnet));
      expect(auditLogger.logCacheOperation).toHaveBeenCalledWith('Subnet', 'hit', expect.any(String), MOCK_SUBNET_ID_FOR_URI_MOCK, expect.any(Object));
    });

    // Error handling test
    it('should handle API error for subnet details', async () => {
      const apiError = new ActualMaasApiError('Subnet API Error', 500, 'subnet_error');
  ...
  describe('SubnetsListResourceHandler', () => {
    let handler: SubnetsListResourceHandler;
    let registeredCallback: HandlerCallbackType;

    beforeEach(async () => {
      await import('../../mcp_resources/schemas/uriPatterns.js');
      const initialCallCount = mockMcpServer.resource.mock.calls.length;
      handler = new SubnetsListResourceHandler(mockMcpServer, mockMaasApiClient);
    ...
    it('should use cache if available for subnets list', async () => {
      ...
      expect(mockMaasApiClient.get).not.toHaveBeenCalled();
      expect(result.contents[0].text).toBe(JSON.stringify(mockSubnets));
      expect(auditLogger.logCacheOperation).toHaveBeenCalledWith('Subnets', 'hit', expect.any(String), undefined, expect.any(Object));
    });
    ...
    it('should handle API error for subnets list', async () => {
      const apiError = new ActualMaasApiError('Subnets API Error', 503, 'service_unavailable');
      mockMaasApiClient.get.mockRejectedValue(apiError);
  ...
  describe('registerSubnetResources', () => {
    it('should register both subnet details and list handlers', () => {
      ...
      registerSubnetResources(mockMcpServer, mockMaasApiClient);
      expect(mockMcpServer.resource).toHaveBeenCalledTimes(2);
      expect(mockMcpServer.resource).toHaveBeenCalledWith('maas_subnet_details', expect.any(ResourceTemplate), expect.any(Function));
      expect(mockMcpServer.resource).toHaveBeenCalledWith('maas_subnets_list', expect.any(ResourceTemplate), expect.any(Function));
      expect(logger.info).toHaveBeenCalledWith('Registered subnet resources with caching:', expect.any(Object));
    });
  });
});