...
describe('TimeBasedCacheStrategy', () => {
  ...
  describe('TTL and Expiration', () => {
    ...
    it('should require a TTL for set()', async () => {
      ...
      const strategyForTTLTest = new TimeBasedCacheStrategy(maxSize);
  ...
  describe('Max Size and Eviction (Oldest Item)', () => {
    ...
    it('should evict the oldest item if maxSize is reached and new item is added', async () => {
      const smallStrategy = new TimeBasedCacheStrategy(2); // maxSize = 2
    ...
    it('should not evict if adding an existing key (update)', async () => {
        const smallStrategy = new TimeBasedCacheStrategy(1); // maxSize = 1
  ...
  describe('Periodic Cleanup', () => {
    ...
    it('should periodically remove expired items', async () => {
      const strategyWithCleanup = new TimeBasedCacheStrategy(maxSize);
      ...
      expect((strategyWithCleanup as any).cache.has('key1')).toBe(false); // Internal check
      ...
      expect((strategyWithCleanup as any).cache.has('key2')).toBe(false);
      expect(strategyWithCleanup.size()).toBe(0);
    });

    it('should stop cleanup interval on dispose', () => {
        const strategyToDispose = new TimeBasedCacheStrategy(maxSize);
