import { z } from 'zod';
import { MaasMachineSchema, GetMachineParamsSchema } from '../../mcp_resources/schemas/machineDetailsSchema.js';
import { 
  readyMachine, 
  deployedMachine, 
  minimalMachine, 
...
describe('Zod Schema Validation Tests', () => {
  describe('Basic Schema Validation', () => {
    ...
    test('should validate GetMachineParamsSchema with valid system_id', () => {
      const result = GetMachineParamsSchema.safeParse({ system_id: 'abc123' });
      expect(result.success).toBe(true);
    });

    test('should fail validation for GetMachineParamsSchema with missing system_id', () => {
      const result = GetMachineParamsSchema.safeParse({});
      expect(result.success).toBe(false);
    });
  });
  ...
  describe('Passthrough Functionality', () => {
    test('should accept and preserve additional properties with passthrough', () => {
      const result = MaasMachineSchema.safeParse(extendedMachine);
      expect(result.success).toBe(true);
      if (result.success) {
    ...
    test('should handle deeply nested additional properties', () => {
      ...
      const result = MaasMachineSchema.safeParse(machineWithNestedExtras);
      expect(result.success).toBe(true);
      if (result.success) {
        // Use type assertion for passthrough properties
        expect((result.data as any).deep_nested.level1.level2.level3).toBe('deeply nested value');
      }
    });

    test('should handle large number of additional properties', () => {
      const machineWithManyProps = { ...readyMachine };
      ...
      const result = MaasMachineSchema.safeParse(machineWithManyProps);
      expect(result.success).toBe(true);
      if (result.success) {
    ...
    test('should handle unusual property names', () => {
      ...
      const result = MaasMachineSchema.safeParse(machineWithUnusualProps);
      expect(result.success).toBe(true);
      if (result.success) {
    ...
    test('should still validate required fields even with passthrough', () => {
      ...
      const result = MaasMachineSchema.safeParse(machineWithExtrasMissingRequired);
      expect(result.success).toBe(false);
    });
  });
  ...
  describe('owner_data Type Implementation', () => {
    test('should validate with simple key-value owner_data', () => {
      ...
      const result = MaasMachineSchema.safeParse(machine);
      expect(result.success).toBe(true);
      if (result.success) {
    ...
    test('should validate with large owner_data object', () => {
      const largeOwnerData: Record<string, any> = {};
  ...
  describe('Error Handling', () => {
    test('should provide detailed error messages for invalid types', () => {
      ...
      const result = MaasMachineSchema.safeParse(invalidTypeMachine);
      expect(result.success).toBe(false);
      if (!result.success) {
        const errorMessages = result.error.errors.map(err => err.message).join(', ');
    ...
    test('should provide path information in error objects', () => {
      ...
      const result = MaasMachineSchema.safeParse(invalidNestedMachine);
      expect(result.success).toBe(false);
      if (!result.success) {
        const errors = result.error.errors;
        
        // Check that errors contain path information
        const domainIdError = errors.find(err => 
          err.path.includes('domain') && err.path.includes('id'));
        const domainNameError = errors.find(err => 
          err.path.includes('domain') && err.path.includes('name'));
    ...
    test('should handle multiple validation errors', () => {
      ...
      const result = MaasMachineSchema.safeParse(multipleErrorsMachine);
      expect(result.success).toBe(false);
      if (!result.success) {
  ...
  describe('Integration with API Patterns', () => {
    test('should validate a machine object from API response', () => {
      ...
      const result = MaasMachineSchema.safeParse(apiResponse);
      expect(result.success).toBe(true);
      if (result.success) {
        ...
        // Use type assertion for passthrough properties
        expect((result.data as any)._links.self.href).toBe('/api/machines/api123');
        expect((result.data as any).resource_uri).toBe('/MAAS/api/2.0/machines/api123/');
      }
    });

    test('should transform data during validation if transform functions are defined', () => {
      // Create a schema with a transform function
      const SchemaWithTransform = MaasMachineSchema.transform(data => ({
        ...data,
        hostname: data.hostname.toUpperCase(),
        transformed: true
      }));
      
      const result = SchemaWithTransform.safeParse(readyMachine);
      expect(result.success).toBe(true);
      if (result.success) {
  ...
  describe('Advanced Schema Features', () => {
    test('should create a custom schema with refined owner_data validation', () => {
      ...
      const CustomMachineSchema = MaasMachineSchema.extend({
        owner_data: z.record(z.union([z.string(), z.number(), z.boolean()])).nullable()
      });
    ...
    test('should demonstrate combining passthrough with strict validation for specific fields', () => {
      ...
      const MixedStrictnessSchema = z.object({
        system_id: z.string(),
        metadata: z.object({
          created_by: z.string(),
          created_at: z.string()
