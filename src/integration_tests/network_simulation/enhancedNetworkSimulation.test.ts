...
import { MaasApiError, MaasApiResponse } from '../../types/index.js';
...
describe('Enhanced Network Simulation Integration Tests', () => {
  ...
  describe('Variable Latency Simulation', () => {
    it('should simulate variable latency based on response size', async () => {
      ...
      // Small response
      (mockClient.get as jest.Mock).mockImplementation(() =>
      ...
      (mockClient.get as jest.Mock).mockImplementation(() =>
        Promise.resolve(Array(2500).fill({ id: 1, name: 'test' }))
      );
  ...
  describe('Bandwidth Limitation Simulation', () => {
    it('should simulate bandwidth limitations affecting response time', async () => {
      ...
      // Small response (1KB)
      (mockClient.get as jest.Mock).mockImplementation(() => Promise.resolve({ id: 1, name: 'test' }));
      ...
      (mockClient.get as jest.Mock).mockImplementation(() => Promise.resolve(Array(5000).fill({ id: 1, name: 'test' })));
  ...
  describe('Connection Drop Simulation', () => {
    ...
    it('should demonstrate handling connection drops with retry logic', async () => {
      ...
      (mockClient.get as jest.Mock).mockImplementation((endpoint, params, signal) => {
