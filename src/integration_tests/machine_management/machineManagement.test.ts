...
import { machines } from '../../__tests__/fixtures/machineResponses.js';
...
describe('Machine Management Features', () => {
  ...
  describe('Machine Deployment', () => {
    it('should deploy a machine', async () => {
      ...
      const machineId = machines[0].system_id;
      
      // Make a request to deploy the machine
      const response = await testEnv.request
        .post('/mcp')
        .send(createToolCallRequest('maas_deploy_machine', {
          system_id: machineId,
          osystem: 'ubuntu',
          distro_series: 'jammy'
        }));
    ...
    it('should deploy a machine with progress notifications', async () => {
      ...
      const response = await testEnv.request
        .post('/mcp')
        .send(createToolCallRequest('maas_deploy_machine_with_progress', {
          system_id: machineId,
          osystem: 'ubuntu',
          distro_series: 'jammy',
      ...
      const resourceContent = response.body.content.find((c: any) => c.type === 'resource');
  ...
  describe('Machine Commissioning', () => {
    it('should commission a machine', async () => {
      ...
      const response = await testEnv.request
        .post('/mcp')
        .send(createToolCallRequest('maas_commission_machine_with_progress', {
          system_id: machineId,
          _meta: { progressToken: 'test-token' }
        }));
