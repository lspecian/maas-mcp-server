...
class MaasApiClient {
    ...
    constructor() {
        ...
        this.oauthClient = new OAuth('', // requestTokenUrl - not used for direct API calls
        '', // accessTokenUrl - not used for direct API calls
        this.consumerKey, '', // consumerSecret - for PLAINTEXT with token, this is often empty.
        '1.0A', // version
    ...
    async makeRequest(method, endpoint, queryParams, body, // BodyInit is a standard Fetch API type, compatible with MaasApiRequestBody
    signal) {
        const MAX_RETRIES = 3;
        const INITIAL_DELAY_MS = 1000;
        const RETRYABLE_STATUS_CODES = new Set([429, 502, 503, 504]);
        const delayWithSignal = (ms, currentSignal) => {
        ...
        let lastError = new MaasApiError(// Default error if all retries fail without a specific last error
        `MAAS API request failed after ${MAX_RETRIES + 1} attempts for ${method} ${url.toString()}`, undefined, 'max_retries_exceeded');
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            ...
            try {
                ...
                if (response.status === 204) { // No Content
                    ...
                    return undefined; // Explicitly cast for undefined response
                }
                ...
                if (contentTypeHeader && contentTypeHeader.includes('application/json')) {
                    try {
                        ...
                        throw new MaasApiError(`Failed to parse JSON response from ${url.toString()}: ${parseError.message}`, response.status, 'json_parse_error', { responseText: responseText.substring(0, 1000), originalError: parseError });
                    }
                }
                ...
                if (signal?.aborted) {
                    ...
                    if (error.name === 'AbortError')
                        throw error; // Rethrow original AbortError
                ...
                logger.error(`MAAS API Request error not retried or max retries reached for network error (Attempt ${attempt + 1}). Error: ${error.message}`, {
                    ...
                    isMaasApiError: error instanceof MaasApiError, statusCode: error.statusCode
                });
        ...
        logger.error(`MAAS API Request failed definitively after ${MAX_RETRIES + 1} attempts for ${method} ${url.toString()}. Last error: ${lastError.message}`, {
            ...
            lastErrorDetails: lastError.details ? JSON.stringify(lastError.details).substring(0, 500) : undefined,
        });
        ...
        throw new MaasApiError(`Network or unexpected error during MAAS API request to ${method} ${url.toString()}: ${lastError.message}`, undefined, lastError.name === 'AbortError' ? 'request_aborted' : 'network_error', { originalErrorName: lastError.name, originalErrorMessage: lastError.message, stack: lastError.stack });
    }
    /**
    ...
    async get(endpoint, params, signal) {
        ...
        return this.makeRequest('GET', endpoint, params, null, signal);
    }
    /**
    ...
    async post(endpoint, data, signal) {
            ...
            for (const key in data) {
                if (Object.prototype.hasOwnProperty.call(data, key)) {
                    ...
                    if (value instanceof Blob) { // Handles File objects as well
                        formData.append(key, value, value.name); // Add filename for File objects
                    }
        ...
        return this.makeRequest('POST', endpoint, undefined, bodyToSend, signal);
    }
    /**
    ...
    async put(endpoint, data, signal) {
        ...
        return this.makeRequest('PUT', endpoint, undefined, bodyToSend, signal);
    }
    /**
     ...
     */
    async delete(endpoint, queryParams, signal) {
        ...
        return this.makeRequest('DELETE', endpoint, queryParams, null, signal);
    }
    /**
     ...
     */
    async postMultipart(endpoint, formData, signal) {
        ...
        try {
            ...
            if (contentTypeHeader && contentTypeHeader.includes('application/json')) {
                try {
                    ...
                    throw new MaasApiError(`Failed to parse JSON response from multipart request to ${endpoint}: ${parseError.message}`, response.status);
                }
            }
            return responseText;
        }
            ...
            throw new MaasApiError(`Network or unexpected error during MAAS API multipart request to ${endpoint}: ${error.message}`, undefined);
        }
    }
}
exports.MaasApiClient = MaasApiClient;
