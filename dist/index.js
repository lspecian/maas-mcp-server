...
// Define the 2024-11-05 protocol version
...
console.log(`LATEST_PROTOCOL_VERSION from SDK: ${LATEST_PROTOCOL_VERSION}`);
console.log(`PROTOCOL_VERSION_2024_11_05: ${PROTOCOL_VERSION_2024_11_05}`);
console.log(`config.mcpUseLatestProtocol: ${config.mcpUseLatestProtocol}`);
console.log(`config.mcpProtocolVersion: ${config.mcpProtocolVersion}`);
...
/**
 * Direct endpoint for listing machines
 *
 * This endpoint bypasses the MCP protocol and directly calls the listMachines tool.
 * It's useful for testing the tool functionality without dealing with MCP protocol issues.
 */
app.get('/api/machines', async (req, res) => {
    try {
        // Call the MAAS API directly using the MaasApiClient
...
app.post('/mcp', async (req, res) => {
    ...
    if (config.auditLogEnabled) {
        ...
        if (isResourceRequest) {
            ...
            auditLogger.logResourceAccess(resourceType, resourceId, 'access', requestId, userId, clientIp, {
            ...
            if (isModification) {
                auditLogger.logResourceModification('tool', toolName, 'execute', requestId, undefined, // beforeState
                ...
                auditLogger.logResourceAccess('tool', toolName, 'execute', requestId, userId, clientIp, {
                    toolName,
                    arguments: toolArgs ? JSON.stringify(toolArgs).substring(0, 200) : undefined
                });
            }
    ...
    try {
        ...
        if (config.auditLogEnabled) {
            ...
            if (isToolRequest) {
                ...
                const isModification = modificationTools.some(tool => toolName.includes(tool));
                if (isModification) {
                    auditLogger.logResourceModification('tool', toolName, 'execute_success', requestId, undefined, // beforeState
                    undefined, // afterState
                    userId, clientIp);
                }
            }
        ...
        if (config.auditLogEnabled) {
            ...
            if (isResourceRequest) {
                ...
                const resourceType = uri.pathname.split('/')[1] || 'unknown';
                const resourceId = uri.pathname.split('/')[2];
                auditLogger.logResourceAccessFailure(resourceType, resourceId, 'access_failure', requestId, error, userId, clientIp, {
                    uri: req.body.params.uri,
                    method: req.body.method
                });
            }
                ...
                const isModification = modificationTools.some(tool => toolName.includes(tool));
                if (isModification) {
                    auditLogger.logResourceModificationFailure('tool', toolName, 'execute_failure', requestId, error, undefined, // beforeState
                    userId, clientIp, {
                        toolName,
                    ...
                    auditLogger.logResourceAccessFailure('tool', toolName, 'execute_failure', requestId, error, userId, clientIp, {
                        toolName,
                        arguments: req.body.params.arguments ? JSON.stringify(req.body.params.arguments).substring(0, 200) : undefined
                    });
                }
...
app.listen(port, () => {
    ...
    logger.info(`Using MCP protocol version: ${protocolVersion}`);
    logger.info(`MCP protocol configuration: useLatest=${config.mcpUseLatestProtocol}, version=${config.mcpProtocolVersion || 'not specified'}`);
});
