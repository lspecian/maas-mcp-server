...
const { createDerivedSignal, onAbort, throwIfAborted, isAbortError, handleAbortError, AbortedOperationError } = require("./abortSignalUtils");
const { registerOperation, unregisterOperation, getOperation, updateOperation } = require("./operationsRegistry");
...
function handleOperationError(error, message) {
    ...
    if (isAbortError(error) || error instanceof AbortedOperationError) {
        throw new MaasServerError(message || 'Operation was aborted', ErrorType.OPERATION_ABORTED, { originalError: error });
    }
    throw new MaasServerError(message || 'Operation failed', ErrorType.OPERATION_FAILED, { originalError: error });
}
module.exports = {
    OperationContext,
