...
function registerUpdateMachineTool(server, maasClient) {
    server.tool("updateMachine", updateMachineSchema.shape, async (params) => {
        ...
        try {
            ...
            if (params.power_type)
                apiParams.power_type = params.power_type;
            if (params.power_parameters)
                apiParams.power_parameters = params.power_parameters;
            ...
            if (error.name === 'ValidationError') {
                ...
                throw new MaasServerError('Failed to update machine: ' + error.message, ErrorType.INTERNAL, { originalError: error.message });
            }
        }
    });
}
