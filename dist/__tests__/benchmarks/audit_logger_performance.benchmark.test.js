...
describe('Audit Logger Performance Benchmarks', () => {
    ...
    afterAll(() => {
        ...
        if (process.env.EXPORT_RESULTS === 'true') {
            ...
            const resultsPath = path_1.default.join(resultsDir, `audit-logger-benchmark-results-${timestamp}.json`);
     ...
     */
    const setupServer = (auditLogConfig) => {
        ...
        config_js_1.default.auditLogEnabled = auditLogConfig.auditLogEnabled;
        config_js_1.default.auditLogIncludeResourceState = auditLogConfig.includeResourceState;
        config_js_1.default.auditLogMaskSensitiveFields = auditLogConfig.maskSensitiveFields;
        config_js_1.default.auditLogToFile = auditLogConfig.logToFile;
     ...
     */
    const runBenchmark = async (testName, auditLogEnabled, includeResourceState, maskSensitiveFields, logToFile, concurrentRequests, totalRequests, networkDelay, requestPattern) => {
     ...
     */
    const printPerformanceImpactSummary = (results) => {
    ...
    describe('Basic Performance Tests', () => {
        it('should measure basic performance impact of audit logging', async () => {
            ...
            for (const networkDelay of [50]) { // Using just one delay for faster tests
                ...
                await runBenchmark(`Basic (${networkDelay}ms delay, audit log disabled)`, false, // auditLogEnabled
                false, // includeResourceState
                true, // maskSensitiveFields
                false, // logToFile
                1, // concurrentRequests
                20, // totalRequests (reduced for faster tests)
                networkDelay, 'same' // requestPattern
                );
                // Run with basic audit logging
                await runBenchmark(`Basic (${networkDelay}ms delay, audit log enabled)`, true, // auditLogEnabled
                false, // includeResourceState
                true, // maskSensitiveFields
                false, // logToFile
                1, // concurrentRequests
    ...
    describe('Resource State Impact Tests', () => {
        it('should measure performance impact of including resource state in audit logs', async () => {
            ...
            for (const networkDelay of [50]) { // Using just one delay for faster tests
                ...
                await runBenchmark(`Resource State (${networkDelay}ms delay, without state)`, true, // auditLogEnabled
                false, // includeResourceState
                true, // maskSensitiveFields
                false, // logToFile
                1, // concurrentRequests
                ...
                await runBenchmark(`Resource State (${networkDelay}ms delay, with state)`, true, // auditLogEnabled
    ...
    describe('Sensitive Field Masking Tests', () => {
        it('should measure performance impact of masking sensitive fields in audit logs', async () => {
            ...
            for (const networkDelay of [50]) { // Using just one delay for faster tests
                ...
                await runBenchmark(`Masking (${networkDelay}ms delay, without masking)`, true, // auditLogEnabled
                true, // includeResourceState
                ...
                await runBenchmark(`Masking (${networkDelay}ms delay, with masking)`, true, // auditLogEnabled
                true, // includeResourceState
                true, // maskSensitiveFields
                false, // logToFile
                1, // concurrentRequests
    ...
    describe('Concurrency Tests', () => {
        it('should measure performance impact of audit logging under different concurrency levels', async () => {
            ...
            for (const concurrentRequests of [1, 5]) { // Reduced for faster tests
                // Run without audit logging
                await runBenchmark(`Concurrency (${concurrentRequests} concurrent, audit log disabled)`, false, // auditLogEnabled
                false, // includeResourceState
                true, // maskSensitiveFields
                false, // logToFile
                concurrentRequests, 20, // totalRequests (reduced for faster tests)
                50, // Fixed network delay
                'same' // requestPattern
                );
                // Run with basic audit logging
                await runBenchmark(`Concurrency (${concurrentRequests} concurrent, audit log enabled)`, true, // auditLogEnabled
                false, // includeResourceState
                true, // maskSensitiveFields
                false, // logToFile
                concurrentRequests, 20, // totalRequests (reduced for faster tests)
                ...
                await runBenchmark(`Concurrency (${concurrentRequests} concurrent, full audit log)`, true, // auditLogEnabled
                true, // includeResourceState
                true, // maskSensitiveFields
                false, // logToFile
                concurrentRequests, 20, // totalRequests (reduced for faster tests)
    ...
    describe('Request Pattern Tests', () => {
        it('should measure performance impact of audit logging with different request patterns', async () => {
            ...
            for (const pattern of patterns) {
                // Run without audit logging
                await runBenchmark(`Pattern (${pattern}, audit log disabled)`, false, // auditLogEnabled
                false, // includeResourceState
                true, // maskSensitiveFields
                false, // logToFile
                5, // Fixed concurrency
                ...
                pattern);
                // Run with basic audit logging
                await runBenchmark(`Pattern (${pattern}, audit log enabled)`, true, // auditLogEnabled
                false, // includeResourceState
                true, // maskSensitiveFields
                false, // logToFile
                5, // Fixed concurrency
                ...
                pattern);
                // Run with full audit logging (resource state + masking)
                await runBenchmark(`Pattern (${pattern}, full audit log)`, true, // auditLogEnabled
                true, // includeResourceState
                true, // maskSensitiveFields
                false, // logToFile
                5, // Fixed concurrency
    ...
    describe('Full Configuration Tests', () => {
        it('should measure performance impact of different audit log configurations', async () => {
            ...
            await runBenchmark(`Configuration (baseline, audit log disabled)`, false, // auditLogEnabled
            false, // includeResourceState
            false, // maskSensitiveFields
            false, // logToFile
            5, // Fixed concurrency
            ...
            for (const config of configurations) {
                ...
                await runBenchmark(`Configuration (${config.name})`, true, // auditLogEnabled
                config.includeState, config.maskFields, false, // logToFile
                5, // Fixed concurrency
                20, // totalRequests (reduced for faster tests)
                50, // Fixed network delay
                'mixed' // requestPattern
