...
const allocateMachine_js_1 = require("../../mcp_tools/allocateMachine.js");
...
jest.mock('../../maas/MaasApiClient.js', () => {
    ...
    const MockMaasApiClient = jest.fn().mockImplementation((_config) => {
        return {
            post: mockMaasPost,
        };
    });
...
const maasClientInstance = new MaasApiClient_js_1.MaasApiClient({
...
beforeAll(() => {
    ...
    (0, allocateMachine_js_1.registerAllocateMachineTool)(server, maasClientInstance);
});
beforeEach(() => {
...
describe('allocateMachine MCP Tool', () => {
    describe('Parameter Transformation', () => {
        it('should transform min_cpu_count, min_memory, min_storage correctly', async () => {
            ...
            expect(mockMaasPost).toHaveBeenCalledWith('/machines', expect.objectContaining({
                op: 'allocate',
                cpu_count: '4',
                mem: '8192',
                storage: '100',
    ...
    describe('Successful Allocation Scenarios', () => {
        ...
        it('should allocate with a combination of constraints', async () => {
            ...
            await toolHandler(params, { sendNotification: mockSendNotification });
            expect(mockMaasPost).toHaveBeenCalledWith('/machines', {
                op: 'allocate',
                name: 'combo-machine-name',
                cpu_count: '2',
                mem: '4096',
    ...
    describe('Error Handling', () => {
        it('should return an error response if MAAS API call fails', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(expect.objectContaining({ error: apiError.message, stack: expect.stringContaining('Error: MAAS API Allocation Failed') }), 'Error allocating machine');
        });
    ...
    describe('Progress Notifications', () => {
        ...
        it('should log a warning if sending notification fails', async () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith({ error: 'Notification send failed' }, 'Failed to send progress notification');
