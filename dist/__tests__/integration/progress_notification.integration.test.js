...
const progressNotification_ts_1 = __importDefault(require("../../utils/progressNotification.ts"));
// @ts-expect-error - CommonJS module imported as ESM
const operationsRegistry_ts_1 = __importDefault(require("../../utils/operationsRegistry.ts"));
// @ts-expect-error - CommonJS module imported as ESM
const operationHandlerUtils_ts_1 = __importDefault(require("../../utils/operationHandlerUtils.ts"));
// @ts-expect-error - CommonJS module imported as ESM
const abortSignalUtils_ts_1 = __importDefault(require("../../utils/abortSignalUtils.ts"));
// Extract the functions we need from the modules
const { sendProgressNotification, createProgressSender, clearRateLimitHistory } = progressNotification_ts_1.default;
const { registerOperation, updateOperation, getOperation, getAllOperations, OperationStatus } = operationsRegistry_ts_1.default;
// Define abortOperation if it doesn't exist in the exports
const abortOperation = operationsRegistry_ts_1.default.abortOperation ||
    ((token, reason) => {
...
const { withOperationHandler } = operationHandlerUtils_ts_1.default;
// Define createOperationContext if it doesn't exist in the exports
const createOperationContext = operationHandlerUtils_ts_1.default.createOperationContext ||
    ((progressToken, operationName, sendNotification) => {
        ...
        return {
            ...
            operationName,
            sendProgress,
            signal: abortSignalUtils_ts_1.default.createDerivedSignal(),
            logger: { debug: console.debug, info: console.info, warn: console.warn, error: console.error },
            operationDetails: operation
        };
    });
const { createDerivedSignal } = abortSignalUtils_ts_1.default;
// Mock MCP server
const mockMcpServer = {
    tool: jest.fn(),
};
...
describe('Progress Notification Integration Tests', () => {
    ...
    describe('End-to-end notification flow', () => {
        it('should handle the complete notification flow for a successful operation', async () => {
            ...
            const sendProgress = createProgressSender(progressToken, mockSendNotification, 'test-request-id', operationType);
        ...
        it('should handle the notification flow for an aborted operation', async () => {
            ...
            const sendProgress = createProgressSender(progressToken, mockSendNotification, 'test-request-id', operationType, undefined, controller.signal);
    ...
    describe('Operation handler integration', () => {
        it('should integrate with operation handler for a successful operation', async () => {
            ...
            const testHandler = async (params, context) => {
            ...
            const wrappedHandler = withOperationHandler('test-wrapped-operation', testHandler, {
                timeout: 30000,
                initialMessage: 'Preparing test operation'
            });
            // 3. Call the wrapped handler
            const progressToken = 'wrapped-operation-token';
            const result = await wrappedHandler({ _meta: { progressToken } }, { signal: undefined, sendNotification: mockSendNotification });
            // 4. Verify the result
        ...
        it('should handle errors in operation handler', async () => {
            ...
            const testErrorHandler = async (params, context) => {
            ...
            // 2. Wrap the handler with operation handler
            const wrappedHandler = withOperationHandler('test-error-operation', testErrorHandler);
            // 3. Call the wrapped handler and expect it to throw
            const progressToken = 'error-operation-token';
            await expect(wrappedHandler({ _meta: { progressToken } }, { signal: undefined, sendNotification: mockSendNotification })).rejects.toThrow('Test operation failed');
    ...
    describe('Operation context integration', () => {
        it('should create and use operation context correctly', () => {
            ...
            const context = createOperationContext(progressToken, operationName, mockSendNotification);
    ...
    describe('Edge cases and error scenarios', () => {
        it('should handle missing progressToken gracefully', async () => {
            ...
            const sendProgress = createProgressSender(undefined, mockSendNotification);
        ...
        it('should handle sendNotification failures gracefully', async () => {
            ...
            }, failingSendNotification, 'test-request-id', 'test-tool');
            ...
            const sendProgress = createProgressSender('failing-sender-token', failingSendNotification, 'test-request-id', 'test-tool');
            // 4. Send notification (should not throw)
            await expect(sendProgress(50, 'This should also fail but not throw')).resolves.not.toThrow();
        });
