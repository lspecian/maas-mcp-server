...
class MockMaasApiClient {
    ...
    _listMachines(params) {
        ...
        if (params) {
            if (params.hostname) {
                filteredMachines = filteredMachines.filter(m => m.hostname.includes(params.hostname));
            }
            ...
            if (params.tag_names) {
                ...
                filteredMachines = filteredMachines.filter(m => tagNames.some((tag) => m.tags.includes(tag)));
            }
            if (params.status) {
            ...
            if (params.zone) {
                filteredMachines = filteredMachines.filter(m => m.zone.name === params.zone);
            }
            if (params.pool) {
                filteredMachines = filteredMachines.filter(m => m.pool.name === params.pool);
            }
            if (params.owner) {
                filteredMachines = filteredMachines.filter(m => m.owner === params.owner);
            }
            if (params.architecture) {
                filteredMachines = filteredMachines.filter(m => m.architecture === params.architecture);
            }
            // Handle pagination
    ...
    listSubnets(params) {
        ...
        if (params) {
            if (params.cidr) {
                filteredSubnets = filteredSubnets.filter(s => s.cidr === params.cidr);
            }
            if (params.name) {
                filteredSubnets = filteredSubnets.filter(s => s.name.includes(params.name));
            }
            if (params.vlan) {
                filteredSubnets = filteredSubnets.filter(s => s.vlan.name === params.vlan);
            }
        }
