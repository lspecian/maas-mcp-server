...
const BaseResourceHandler_ts_1 = require("../../mcp_resources/BaseResourceHandler.ts");
...
const logger = __importStar(require("../../utils/logger.ts"));
const auditLogger_ts_1 = __importDefault(require("../../utils/auditLogger.ts"));
...
class TestResourceHandler extends BaseResourceHandler_ts_1.BaseResourceHandler {
    constructor(server, maasClient) {
        super(server, maasClient, 'TestResource', {}, '/test/:id', zod_1.z.any(), zod_1.z.object({ id: zod_1.z.string() }), '/api/test');
    }
    ...
    async testHandleRequest(uri, variables, options) {
...
describe('Request ID Generation in BaseResourceHandler', () => {
    ...
    describe('Uniqueness and Format', () => {
        it('should generate unique request IDs for each request', async () => {
            ...
            await resourceHandler.testHandleRequest(new URL('http://example.com/test/123'), { id: '123' }, { signal: new AbortController().signal });
    ...
    describe('Inclusion in Logs', () => {
        it('should include request ID in audit logs for handleRequest', async () => {
            ...
            expect(auditLogger_ts_1.default.logResourceAccess).toHaveBeenCalled();
            const calls = auditLogger_ts_1.default.logResourceAccess.mock.calls;
            // Find a call that matches our expected pattern
            const matchingCall = calls.find(call => call[0] === 'TestResource' &&
                call[2] === 'read' &&
                call[3] === 'test-request-id');
            expect(matchingCall).toBeTruthy();
            // Verify that cache operation logs include the request ID
            expect(auditLogger_ts_1.default.logCacheOperation).toHaveBeenCalledWith('TestResource', 'miss', 'test-request-id', '123', expect.any(Object));
            expect(auditLogger_ts_1.default.logCacheOperation).toHaveBeenCalledWith('TestResource', 'set', 'test-request-id', '123', expect.any(Object));
        });
        it('should include request ID in audit logs for invalidateCache', () => {
            ...
            resourceHandler.invalidateCache();
            // Verify that audit logs include the request ID
            expect(auditLogger_ts_1.default.logCacheOperation).toHaveBeenCalledWith('TestResource', 'invalidate_all', 'cache-invalidate-id', undefined, { count: 5 });
        });
        it('should include request ID in audit logs for invalidateCacheById', () => {
            ...
            resourceHandler.invalidateCacheById('123');
            // Verify that audit logs include the request ID
            expect(auditLogger_ts_1.default.logCacheOperation).toHaveBeenCalledWith('TestResource', 'invalidate_by_id', 'cache-invalidate-by-id', '123', { count: 2 });
        });
        it('should include request ID in audit logs for setCacheOptions', () => {
            ...
            resourceHandler.setCacheOptions({ ttl: 600 });
            // Verify that audit logs include the request ID
            expect(auditLogger_ts_1.default.logCacheOperation).toHaveBeenCalledWith('TestResource', 'update_options', 'set-cache-options-id', undefined, expect.objectContaining({
                cacheTTL: 600
            }));
        });
    });
    describe('Correlation of Request IDs', () => {
        it('should use the same request ID for all logs related to a single request', async () => {
            ...
            cacheManager.CacheManager.getInstance().get.mockReturnValue(null);
            // Call handleRequest
            await resourceHandler.testHandleRequest(new URL('http://example.com/test/123'), { id: '123' }, { signal: new AbortController().signal });
            // Verify that all audit logs for this request use the same request ID
            const logCalls = auditLogger_ts_1.default.logCacheOperation.mock.calls;
            ...
            const resourceAccessCalls = auditLogger_ts_1.default.logResourceAccess.mock.calls;
        ...
        it('should use different request IDs for different operations', async () => {
            ...
            const handleRequestCalls = logCalls.filter(call => call[1] === 'miss' || call[1] === 'set');
            const invalidateCacheCalls = logCalls.filter(call => call[1] === 'invalidate_all');
            const invalidateCacheByIdCalls = logCalls.filter(call => call[1] === 'invalidate_by_id');
    ...
    describe('Edge Cases', () => {
        it('should handle aborted requests gracefully', async () => {
            ...
            expect(auditLogger_ts_1.default.logResourceAccessFailure).toHaveBeenCalled();
            const calls = auditLogger_ts_1.default.logResourceAccessFailure.mock.calls;
        ...
        it('should handle cache operations when caching is disabled', async () => {
            ...
            // Mock cache disabled
            const cacheManager = await import('../../mcp_resources/cache/index.js');
            cacheManager.CacheManager.getInstance().isEnabled.mockReturnValue(false);
            ...
            expect(auditLogger_ts_1.default.logCacheOperation).not.toHaveBeenCalled();
            ...
            expect(auditLogger_ts_1.default.logCacheOperation).toHaveBeenCalledWith('TestResource', 'update_options', 'cache-disabled-id', undefined, expect.objectContaining({
                cacheEnabled: false
            }));
        });
    });
