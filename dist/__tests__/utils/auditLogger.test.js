...
describe('Audit Logger', () => {
    ...
    describe('Sensitive Data Handling', () => {
        it('should mask sensitive fields in objects', async () => {
            ...
            logResourceAccess('machine', '123', 'view', 'req-123', 'user-456', '192.168.1.1', { detail: 'test' }, resourceState);
        ...
        it('should mask sensitive fields in deeply nested objects and arrays', async () => {
            ...
            logResourceModification('user', '123', 'update-complex-data', 'req-123', beforeState, afterState);
            // Check that the log was called with masked data
            expect(mockChildLogger.info).toHaveBeenCalledTimes(1);
            const logCall = mockChildLogger.info.mock.calls[0];
            const logEntry = logCall[0];
        ...
        it('should handle arrays in sensitive data masking', async () => {
            ...
            logResourceModification('user', '456', 'update-credentials', 'req-456', beforeState, afterState);
    ...
    describe('Event Types', () => {
        it('should log resource access events correctly', async () => {
            ...
            logResourceAccess('machine', '123', 'view', 'req-123', 'user-456', '192.168.1.1', { detail: 'test' });
            expect(mockChildLogger.info).toHaveBeenCalledTimes(1);
            const logCall = mockChildLogger.info.mock.calls[0];
            const [logEntry, logMessage] = logCall;
            // Verify log entry structure
            expect(logEntry).toMatchObject({
                eventType: auditLogger_js_1.AuditEventType.RESOURCE_ACCESS,
        ...
        it('should log resource access failure events correctly', async () => {
            ...
            logResourceAccessFailure('machine', '123', 'view', 'req-123', error, 'user-456', '192.168.1.1', { detail: 'test' });
        ...
        it('should log resource modification events correctly', async () => {
            ...
            logResourceModification('machine', '123', 'update-status', 'req-123', beforeState, afterState, 'user-456', '192.168.1.1', { detail: 'test' });
            expect(mockChildLogger.info).toHaveBeenCalledTimes(1);
            const logCall = mockChildLogger.info.mock.calls[0];
            const [logEntry, logMessage] = logCall;
            // Verify log entry structure
            expect(logEntry).toMatchObject({
                eventType: auditLogger_js_1.AuditEventType.RESOURCE_MODIFICATION,
                resourceType: 'machine',
                resourceId: '123',
        ...
        it('should log resource modification failure events correctly', async () => {
            ...
            const beforeState = { status: 'inactive' };
            logResourceModificationFailure('machine', '123', 'update-status', 'req-123', error, beforeState, 'user-456', '192.168.1.1', { detail: 'test' });
            expect(mockChildLogger.error).toHaveBeenCalledTimes(1);
            const logCall = mockChildLogger.error.mock.calls[0];
            const [logEntry, logMessage] = logCall;
            // Verify log entry structure
        ...
        it('should log cache operation events correctly', async () => {
            ...
            logCacheOperation('machine', 'hit', 'req-123', '123', { cacheKey: 'machine:123' });
            expect(mockChildLogger.info).toHaveBeenCalledTimes(1);
            const logCall = mockChildLogger.info.mock.calls[0];
            const [logEntry, logMessage] = logCall;
            // Verify log entry structure
            expect(logEntry).toMatchObject({
                eventType: auditLogger_js_1.AuditEventType.CACHE_OPERATION,
                resourceType: 'machine',
                resourceId: '123',
                action: 'hit',
                status: 'success',
    ...
    describe('Resource Creation and Deletion Events', () => {
        it('should log resource creation events correctly', async () => {
            ...
            logResourceModification(resourceType, resourceId, action, requestId, undefined, // No before state for creation
            afterState, userId, ipAddress, details);
            expect(mockChildLogger.info).toHaveBeenCalledTimes(1);
            const logCall = mockChildLogger.info.mock.calls[0];
            const [logEntry, logMessage] = logCall;
            // Verify log entry structure
        ...
        it('should log resource deletion events correctly', async () => {
            ...
            logResourceModification(resourceType, resourceId, action, requestId, beforeState, undefined, // No after state for deletion
    ...
    describe('Error Handling', () => {
        it('should handle non-Error objects in error details', async () => {
            ...
            logResourceAccessFailure('machine', '123', 'view', 'req-123', error);
            expect(mockChildLogger.error).toHaveBeenCalledTimes(1);
            const logCall = mockChildLogger.error.mock.calls[0];
            const logEntry = logCall[0];
        ...
        it('should handle null or undefined errors gracefully', async () => {
            ...
            logResourceAccessFailure('machine', '123', 'view', 'req-123', undefined);
            expect(mockChildLogger.error).toHaveBeenCalledTimes(1);
            ...
            logResourceAccessFailure('machine', '123', 'view', 'req-123', null);
            expect(mockChildLogger.error).toHaveBeenCalledTimes(1);
        ...
        it('should handle undefined resource ID', async () => {
            const { logResourceAccess } = await import('../../utils/auditLogger.js');
            logResourceAccess('machines', // Collection resource type
    ...
    describe('Log Format', () => {
        it('should include ISO timestamp format', async () => {
            const { logResourceAccess } = await import('../../utils/auditLogger.js');
            logResourceAccess('machine', '123', 'view', 'req-123');
            expect(mockChildLogger.info).toHaveBeenCalledTimes(1);
            const logCall = mockChildLogger.info.mock.calls[0];
            const logEntry = logCall[0];
        ...
        it('should handle optional parameters correctly', async () => {
            ...
            expect(logEntry.eventType).toBe(auditLogger_js_1.AuditEventType.RESOURCE_ACCESS);
