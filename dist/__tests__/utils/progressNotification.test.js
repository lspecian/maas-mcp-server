...
// @ts-nocheck
const { sendProgressNotification, createProgressSender, clearRateLimitHistory, DEFAULT_RATE_LIMIT_CONFIG } = require('../../utils/progressNotification');
...
describe('progressNotification', () => {
    ...
    describe('sendProgressNotification', () => {
        ...
        it('should log a warning if sending notification fails', async () => {
            ...
            const logger = createRequestLogger.mock.results[0].value;
            expect(logger.warn).toHaveBeenCalledWith({ error: 'Test error', progressToken: 'test-token', progress: 50, total: 100 }, 'Failed to send progress notification');
        });
    });
    ...
    describe('createProgressSender', () => {
        ...
        it('should allow overriding the AbortSignal in the progress sender', async () => {
            ...
            const sendProgress = createProgressSender(progressToken, mockSendNotification, 'test-request-id', 'test-tool', DEFAULT_RATE_LIMIT_CONFIG, controller1.signal);
    ...
    describe('rate limiting', () => {
        ...
        it('should allow important notifications to bypass rate limiting', async () => {
            ...
            await sendProgressNotification({ ...params, message: 'Important notification', important: true }, mockSendNotification);
