...
const { sendProgressNotification, createProgressSender, clearRateLimitHistory, DEFAULT_RATE_LIMIT_CONFIG } = progressNotification_js_1.default;
describe('progressNotification (deterministic tests)', () => {
    ...
    beforeEach(() => {
        ...
        mockSendNotification = globals_1.jest.fn().mockImplementation(() => Promise.resolve());
    ...
    describe('rate limiting', () => {
        it('should rate limit notifications based on time interval', async () => {
            // Set initial time
            globals_1.jest.setSystemTime(new Date(2025, 0, 1, 12, 0, 0));
            const params = {
            ...
            globals_1.jest.setSystemTime(new Date(2025, 0, 1, 12, 0, 0, 500));
            // Second notification should be rate limited
            await sendProgressNotification({ ...params, message: 'Second notification' }, mockSendNotification);
            // mockSendNotification should not have been called
            expect(mockSendNotification).not.toHaveBeenCalled();
            // Advance time beyond the rate limit
            globals_1.jest.setSystemTime(new Date(2025, 0, 1, 12, 0, 1, 100));
            // Third notification should be sent
            await sendProgressNotification({ ...params, message: 'Third notification' }, mockSendNotification);
            expect(mockSendNotification).toHaveBeenCalledTimes(1);
        });
        ...
        it('should respect custom rate limit configuration', async () => {
            ...
            globals_1.jest.setSystemTime(new Date(2025, 0, 1, 12, 0, 1, 500));
            // Second notification should still be rate limited with custom config
            await sendProgressNotification({ ...params, message: 'Second notification' }, mockSendNotification, undefined, undefined, customRateLimit);
            // mockSendNotification should not have been called
            expect(mockSendNotification).not.toHaveBeenCalled();
            // Advance time beyond the custom rate limit
            globals_1.jest.setSystemTime(new Date(2025, 0, 1, 12, 0, 2, 100));
            // Third notification should be sent
            await sendProgressNotification({ ...params, message: 'Third notification' }, mockSendNotification, undefined, undefined, customRateLimit);
            expect(mockSendNotification).toHaveBeenCalledTimes(1);
        });
    });
    describe('abort signal support', () => {
        ...
        it('should check if aborted before sending notification', async () => {
            ...
            (0, deterministicTestUtils_1.advanceTimersByTime)(100);
    ...
    describe('createProgressSender', () => {
        it('should create a function that sends progress notifications with deterministic behavior', async () => {
            ...
            const sendProgress = createProgressSender(progressToken, mockSendNotification, requestId, toolName, noRateLimitConfig);
        ...
        it('should support AbortSignal in the progress sender with deterministic behavior', async () => {
            ...
            const controller = new AbortController();
            const sendProgress = createProgressSender(progressToken, mockSendNotification, 'test-request-id', 'test-tool', DEFAULT_RATE_LIMIT_CONFIG, controller.signal);
            ...
            (0, deterministicTestUtils_1.advanceTimersByTime)(10);
            await syncPoint.wait();
