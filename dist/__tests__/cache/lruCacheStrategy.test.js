...
describe('LRUCacheStrategy', () => {
    ...
    describe('Core Operations', () => {
        ...
        beforeEach(() => {
            ...
            strategy = new lruCacheStrategy_js_1.LRUCacheStrategy(maxSize);
        });
        ...
        it('should clear all keys', () => {
            ...
            expect(strategy.lruOrder.length).toBe(0);
        });
    });
    describe('LRU Eviction Policy', () => {
        let strategy;
        beforeEach(() => {
            strategy = new lruCacheStrategy_js_1.LRUCacheStrategy(maxSize); // maxSize = 3
        });
        it('should evict the least recently used item when maxSize is reached', () => {
            ...
            expect(strategy.get('key3')?.value).toEqual({ data: 'value3' });
            expect(strategy.cache.size).toBe(3);
    ...
    describe('TTL Interaction with LRU', () => {
        let strategy;
        beforeEach(() => {
            strategy = new lruCacheStrategy_js_1.LRUCacheStrategy(maxSize); // defaultTTL is not part of constructor
        });
        it('should remove an item if it expires, regardless of LRU order', () => {
            ...
            expect(strategy.cache.size).toBe(2); // key2 removed
            expect(strategy.lruOrder.includes('key2')).toBe(false);
        });
        it('get should return undefined for expired item and remove it', () => {
            ...
            expect(strategy.lruOrder.includes('key1')).toBe(false);
        });
    });
    describe('Periodic Cleanup of Expired Items (if applicable)', () => {
        ...
        it('should remove an expired item during set if it would have been LRU candidate', () => {
            const lruStrategy = new lruCacheStrategy_js_1.LRUCacheStrategy(2); // maxSize 2
            ...
            expect(lruStrategy.cache.size).toBe(2);
        });
    });
    describe('Dispose Method', () => {
        ...
        it('should clear the cache on dispose', () => {
            const strategyToDispose = new lruCacheStrategy_js_1.LRUCacheStrategy(maxSize);
            strategyToDispose.set('key1', { data: 'value1' }, defaultTTL);
            strategyToDispose.dispose();
            expect(strategyToDispose.cache.size).toBe(0);
            expect(strategyToDispose.lruOrder.length).toBe(0);
