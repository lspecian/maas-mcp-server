...
describe('MaasApiClient', () => {
    ...
    beforeEach(() => {
        ...
        fetchMockFn = require('node-fetch');
    ...
    test('should initialize with correct API URL and key components', () => {
        ...
        expect(client['maasApiUrl']).toBe('https://example.com/MAAS');
        expect(client['consumerKey']).toBe('consumer_key');
        expect(client['oauthToken']).toBe('token');
        expect(client['oauthTokenSecret']).toBe('secret');
    });
    ...
    test('should include OAuth headers in requests', async () => {
        ...
        const paramsArray = paramsStr.split(', ').map((p) => p.split('=')[0]);
    ...
    test('should throw MaasApiError for error responses', async () => {
        ...
        expect(loggerMock.error).toHaveBeenCalledWith(expect.objectContaining({ status: 400, error: 'API Error'.substring(0, 500) }), // Match client's substring
        'MAAS API Error');
    });
    test('should return null for 204 No Content responses', async () => {
        ...
        expect(loggerMock.debug).toHaveBeenCalledWith(expect.objectContaining({ responseStatus: 204 }), 'MAAS API request successful (No Content)');
    });
    test('should handle generic network errors', async () => {
        ...
        expect(loggerMock.error).toHaveBeenCalledWith(expect.objectContaining({ error: 'Simulated network error' }), 'Fetch request failed');
    });
...
describe('FormData handling for MaasApiClient', () => {
    ...
    beforeEach(() => {
        client = new MaasApiClient_js_1.MaasApiClient(); // Align with 0-arg constructor and other tests
    ...
    it('post method should correctly process FormData from "form-data" package', async () => {
        ...
        // @ts-expect-error - request is private
        const requestSpy = client.request;
    ...
    it('post method should convert plain object to FormData and use getBuffer', async () => {
        ...
        // @ts-expect-error - request is private
        const requestSpy = client.request;
        const mockFormDataInstance = new form_data_1.default();
        ...
        global.FormData = jest.fn(() => mockFormDataInstance); // Make client's internal `new FormData()` use our mock
